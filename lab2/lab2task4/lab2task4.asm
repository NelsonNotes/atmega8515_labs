...
;Памятка: в таблице векторов прерываний держим команду безусловного перехода на сам обработчик, который лежит в другом месте
;Памятка: чтобы переместить таблицу векторов прерываний в начало секции загрузчика Boot Flash, нужно проставить бит BOOTRST и IVSEL регистра MCUCR
;BOOTRST перемещает всю прогу после загрузчика, а IVSEL размещает там таблицу векторов прерываний (если не ошибаюсь)
